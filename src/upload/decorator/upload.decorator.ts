...
import {applyDecorators, Controller, MethodNotAllowedException, Post, UploadedFile, UseInterceptors } from '@nestjs/common';
...
export function Upload(field?:any,options?:MulterOptions){
    return applyDecorators(
        UseInterceptors(FileInterceptor(field?field:'file',options))
    ) 
}   
//-----根据返回装饰器定义各类文件类型
